...
THREEx.ProceduralCity = function() {
  ...
  var numOfBldg    = config.cityScape.numOfBldg
    , cityRange    = config.cityScape.cityRange
    , distFrmCent  = config.cityScape.distFrmCent
    ;
  ...
  var cityGeometry= new THREE.Geometry();  //main geometry for the whole city


  for( var i = 0; i < numOfBldg; i ++ ){
    // put a random position
    //var randX = Math.floor( Math.random() * 300 - 100 ) * 10;

    var randX = 0
      , randZ = 0
      ;

    while(lineDistance({x: 0, y: 0}, {x: randX, y: randZ}) < distFrmCent) {
      randZ = random(-cityRange, cityRange);
      randX = random(-cityRange, cityRange);
    }

    //console.log('distance = ' + lineDistance({x: 0, y: 0}, {x: randX, y: randZ}));

    buildingMesh.position.x = randX;
    buildingMesh.position.z = randZ;
